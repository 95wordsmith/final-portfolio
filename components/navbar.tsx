...
const [active,setActive]= useState('')
  ...
  <motion.nav transition={{delay:0.2}} variants={navVariant} initial='outside' animate='inside' className=" flex z-50 w-full fixed px-8 md:px-20 lg:px-30  xl:px-36 top-8  justify-between items-center">
   ...
   <div className=" hidden md:flex md:gap-4 lg:gap-8 xl:gap-10  text-xl">
    {navLinks.map((nav)=>(
      <Link onClick={()=>setActive(nav.title)} className={`${active===nav.title?'text-secondary tracking-wide text-xl font-semibold':'text-primary'} hover:text-secondary tracking-wide `}  key={nav.id} href={nav.link}>{nav.title}</Link>
    ))}
   </div>
   ...
   <div className=" flex flex-col items-center  gap-16 text-3xl pt-20">
    {navLinks.map((nav)=>(
      <SheetClose asChild key={nav.id}>
        <Link onClick={()=>setActive(nav.title)} className={`hover:text-secondary text-center tracking-wide border-secondary border-b-4 hover:border-primary w-full text-primary text-3xl`} key={nav.id} href={nav.link}>{nav.title}</Link>
      </SheetClose>
    ))}
